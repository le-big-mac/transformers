    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            if inputs_embeds is not None:  # Exception 4
                inputs_embeds = inputs_embeds[:, -cache_position.shape[0] :]
            elif (is_torchdynamo_compiling() or cache_position[-1] >= input_ids.shape[1]):  # Exception 3
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        Return:
            ...
            `model.config.is_encoder_decoder=True`.
        """
        want_latent = getattr(self.config, "latent_thinking", False)
        if want_latent:
            generation_config.output_hidden_states = True
            latent_mode = model_kwargs.pop("latent_mode", None)
            if latent_mode is None:
                latent_mode = torch.zeros(input_ids.shape[0], dtype=torch.bool, device=input_ids.device)

            BOT_TOKEN_ID = self.config.bot_token_id
            EOT_TOKEN_ID = self.config.eot_token_id
            bot_in_prompt = (input_ids[:, -1] == BOT_TOKEN_ID)
            latent_mode |= bot_in_prompt

            model_kwargs["inputs_embeds"] = self.get_input_embeddings()(input_ids)
        ...
        while self._has_unfinished_sequences(
            ...
            # ### NEW LATENT THINKING OVERRIDE ###
            if want_latent:
                # update which sequences are in latent mode based on newly sampled token
                is_bot = (next_tokens == BOT_TOKEN_ID)
                is_eot = (next_tokens == EOT_TOKEN_ID)
                latent_mode = (latent_mode & ~is_eot) | is_bot

                # unify embeddings for next step
                # We do a normal embedding lookup for all sequences:
                embed_lookup = self.get_input_embeddings()(next_tokens)  # shape [B, hidden_dim]

                # Then for those in latent_mode, we overwrite with final hidden state
                #   but only if outputs.hidden_states exist
                if outputs.hidden_states is None:
                    # if user didn't actually produce hidden_states, we can't do latent logic
                    raise ValueError(
                        "latent_thinking=True requires output_hidden_states=True, but hidden_states was None."
                    )
                last_hidden = outputs.hidden_states[-1][:, -1, :]  # [B, hidden_dim]
                embed_lookup[latent_mode] = last_hidden[latent_mode]

                # shape => [B, 1, hidden_dim], pass it in
                input_embeds = model_kwargs["inputs_embeds"]
                model_kwargs["inputs_embeds"] = torch.cat([input_embeds, embed_lookup.unsqueeze(1)], dim=1)
                # model_kwargs["input_ids"] = None
                # keep track of the latent_mode array for next iteration
                model_kwargs["latent_mode"] = latent_mode
